...
S3.prototype.makeRequest = function(path, verb, headers, cb, body, params, retries){
    ...
    // log.silly('options and headers: ' +  JSON.stringify(opts, null, 4) + JSON.stringify(headers, null, 4));

    this.path += "?"+querystring.stringify(params);
    log.silly('Making request ' + path + '.  Retries: ' + retries);
    ...
    req = https.request(opts, function(res){
        ...
        res.on('end', function(){
            log.silly('Got end for path '+path, res.statusCode);
            var result = response,
                resultObj = {},
                err = null;
            if(res.statusCode >= 400){
                if(response.length > 0){
                    try{
                        resultObj = common.xmlToObject(response);
                        err = new S3Error(resultObj.error.message, opts);
                    }
                    catch(e){
                        err = e;
                    }
                    ...
                    err = new S3Error('Unknown');
                }
            }
            if(err !== null){
                log.error('Error. Retrying. ' + opts.path, err);
                setTimeout(function(){
                    return self.makeRequest(path, verb, headers, cb, body, params, retries + 1).then(function(){
                        log.silly('Retry succeeded! Resolving. ' + opts.path);
                        d.resolve(arguments[0]);
                    });
                }, 100);
            }
            else{
                log.silly(opts.path + ' Completed.  Calling cb.');
                result = cb.apply(this, [err, res, req]);
                log.silly(opts.path + ' Resolving');
    ...
    req.on('error', function(e) {
        // log.error('Request Error: ' + opts.path, e);
    });
...
S3.prototype.objectExists = function(bucketName, key){
    var d = when.defer();
    this.lookup(bucketName, key).then(function(headers){
        d.resolve((headers !== null));
    });
    return d.promise;
};
