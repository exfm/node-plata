...
SNS.prototype.listSubscriptions = function(nextToken){
    ...
    return this.request('/')
        ...
        .send(params)
        .end(function(res){
            res.subscriptions = res.listSubscriptionsResponse.listSubscriptionsResult;
            return res;
        });
};
...
SNS.prototype.getTopic = function(name){
    return this.listTopics().then(function(res){
...
Topic.prototype.middleware = function(req, res, next){
    ...
    if(!type){
        return next(new Error('Missing x-amz-sns-message-type request header.'));
    }
    ...
    req.on('end', function(){
        try {
            ...
            req.subscriptionArn = req.headers['x-amz-sns-subscription-arn'];
