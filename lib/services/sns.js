...
SNS.prototype.listSubscriptions = function(nextToken){
    ...
    return this.request('/')
        ...
        .send(params)
        .end(function(res){
            res.subscriptions = res.listSubscriptionsResponse.listSubscriptionsResult.subscriptions;
            return res;
        });
};
...
SNS.prototype.listSubscriptionsByTopic = function(arn, nextToken){
    ...
    return this.request('/')
        ...
        .send(params)
        .end(function(res){
            log.silly('listSubscriptionsByTopic got ' + util.inspect(res.listSubscriptionsByTopicResponse.listSubscriptionsByTopicResult, false, 5, false));
            res.subscriptions = res.listSubscriptionsByTopicResponse.listSubscriptionsByTopicResult.subscriptions;
            return res;
        });
};
...
Topic.prototype.getSubscriptions = function(){
    ...
    return this.connection.listSubscriptionsByTopic(this.arn).then(function(res){
        log.silly('listSubscriptionsByTopic returned ' + util.inspect(res.subscriptions, false, 5, false));
...
Topic.prototype.getSubscriptionArn = function(endpoint){
    ...
    this.getSubscriptions().then(function(subs){
        log.silly('Topic.getSubscriptions returned ' + util.inspect(subs, false, 5, false));
        for(var i = 0; i < subs.length; i++){
            if(subs[i].endpoint === endpoint){
                return d.resolve(subs[i].endpoint);
            }
        }
...
// // And then of course we'll get an event for the unsubscription.
...
Topic.prototype.middleware = function(req, res, next){
    ...
    return function(req, res, next){
        ...
        // Not ours.
        if(req.headers['x-amz-sns-topic-arn'] !== this.arn){
