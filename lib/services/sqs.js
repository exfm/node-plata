...
SQS.prototype.sendMessageBatch = function(url, batch) {
    ...
    batch.forEach(function(message, index){
        params['SendMessageBatchRequestEntry.' + (index + 1) + '.Id'] = uuid.v4();
        params['SendMessageBatchRequestEntry.' + (index + 1) + '.MessageBody'] = JSON.stringify(message);
    });

    return this.makeRequest(function(response){
        return response.sendMessageBatchResponse.sendMessageBatchResult;
...
MessageBatch.prototype.put = function(){
    var d = when.defer(),
        ...
        when.all(chunks.map(function(chunk){
            return self.queue.putBatch(chunk);
        }),
            function(results){
                var all = [];
