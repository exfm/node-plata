...
function fromBase64(str){
    ...
    try{
        return JSON.parse(decodeURIComponent(buf.toString('utf-8')));
    }
    catch(e){
        log.error('Couldnt parse message!!!!');
        log.error(buf.toString('utf-8'));
        log.error(decodeURIComponent(buf.toString('utf-8')));
        throw e;
    }
}
...
Queue.prototype.listen = function(interval, cb){
    ...
    if(this._ready){
        ...
        self.get(1).then(function(message){
            self.log.silly('Get(1) returned ' + util.inspect(message, true, 5, false));
            if(message){
            ...
            self.log.error('WHOA.  get(1) rejected.' + util.inspect(arguments, true, 5, false));
        });
    }
    else{
